...
export function ExpiredAccountsModule() {
  const [expiredAccounts, setExpiredAccounts] = useState<User[]>([])
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setExpiredAccounts(filteredAccounts)
      } catch (err) {
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      headers.join(","),
      ...expiredAccounts.map((account) => `${account.name},${account.email},${account.end_date || "No end date"}`),
    ].join("\n")

    const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" })
    saveAs(blob, "expired_accounts.csv")
  }
    ...
    <Card className="bg-gray-900 border-gray-800 rounded-xl overflow-hidden">
      <CardHeader className="border-b border-gray-800 p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <CardTitle className="text-base font-normal text-white">Expired Accounts</CardTitle>
      ...
      <CardContent className="p-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-8">
            ...
            <p className="mt-4 text-gray-400 text-lg">Loading expired accounts...</p>
          </div>
        ) : error ? (
          ...
          <>
            <div className="mb-4">
              <p className="text-xl font-bold text-white">Total Expired Accounts: {expiredAccounts.length}</p>
            </div>
            {expiredAccounts.length > 0 ? (
              <>
                <div className="overflow-x-auto">
                  <Table className="border-collapse">
                    ...
                    <TableBody>
                      {(showAll
                        ? expiredAccounts.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
                        : expiredAccounts.slice(0, 5)
                      ).map((account, index) => (
                ...
                {!showAll && expiredAccounts.length > 5 && (
                ...
                {showAll && (
                  <div className="mt-4 flex justify-between items-center">
                    ...
                    <span className="text-gray-300">
                      Page {currentPage} of {Math.ceil(expiredAccounts.length / itemsPerPage)}
                    </span>
                    <Button
                      onClick={() =>
                        setCurrentPage((prev) => Math.min(prev + 1, Math.ceil(expiredAccounts.length / itemsPerPage)))
                      }
                      disabled={currentPage === Math.ceil(expiredAccounts.length / itemsPerPage)}
                      className="bg-indigo-600 text-white hover:bg-indigo-700 border-0 disabled:bg-gray-700"
                    >
                      Next
                    </Button>
              ...
              <p className="text-center text-gray-400 mt-4">No expired accounts found.</p>
            )}
          </>
        )}
      </CardContent>
